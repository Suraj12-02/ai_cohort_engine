...
with tab_bi:
    ...
    # Debug: show the data returned by the database
    st.write("DEBUG CLV DATA:")
    st.dataframe(
        clv_df,
    ...
    with c1:
        if not clv_df.empty:
            ...
            st.warning("No CLV data returned from the database.")
    ...
    repeat_pct = (
        repeat_df.iloc[0, 0]
        if not repeat_df.empty and repeat_df.iloc[0, 0] is not None
        else 0
    )
    ...
    with c1:
        fig = px.bar(
            ...
            text_auto=".1f",
            labels={
                "churn_risk_pct": "% at churn risk (no order in 90d)",
    ...
    with c1:
        fig = px.bar(
            ...
            text_auto=".2s",
            labels={
                "attributed_revenue": "Attributed revenue ($, 7-day window)",
    ...
    with st.spinner(
        "Loading retention cohort (can take a moment on large datasets)..."
    ):
    ...
    if not retention_df.empty:
        ...
        st.info(
            "Not enough order history yet to build a retention cohort."
        )
...
with tab_copilot:
    ...
    if audit_col.button(
        ...
        with st.spinner(
            "Running static analysis + LLM review..."
        ):
        ...
        score_color = (
            "green"
            if report.score >= 80
            else ("orange" if report.score >= 50 else "red")
        )

        st.markdown(
            f"**Performance score:** :{score_color}[{report.score}/100]"
        )
        ...
        if report.findings:
            for f in report.findings:
                ...
                st.markdown(
                    f"{icon} **[{f.severity.upper()}] {f.rule}** — {f.message}"
                )

        else:
            st.success(
                "No rule-based anti-patterns detected."
            )
    ...
    if exec_col.button(
        "▶️ Run EXPLAIN ANALYZE",
        use_container_width=True
    ):
        try:
            with st.spinner(
                "Executing query and capturing plan..."
            ):
                plan_json, latency = explain_analyze(
                    sql_input
                )
            ...
            with st.expander(
                "Full EXPLAIN (ANALYZE, BUFFERS, JSON) plan"
            ):
                st.json(
                    plan_json
                )
    ...
    st.subheader(
        "⚡ Before/After Indexing — Latency Comparison"
    )

    if BENCHMARK_RESULTS_PATH.exists():
        data = json.loads(
            BENCHMARK_RESULTS_PATH.read_text()
        )
        ...
        if before:
            ...
            fig = px.bar(
                ...
                log_y=True,
                labels={
                    "latency_ms": "Latency (ms, log scale)",
            ...
            st.info(
                "Run `python -m src.query_benchmarker --run-all` "
